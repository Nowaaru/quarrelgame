...
import { CharacterController2D, Entity, EntityAttributes, Map, MatchController, MatchPhase, MatchSettings, MatchState, OnArenaChange, OnMatchRespawn, OnMatchStart, ParticipantAttributes } from "@rbxts/quarrelgame-framework";
import { MotionInput } from "@rbxts/quarrelgame-framework";
import { Players } from "@rbxts/services";
import { Combat } from "./combat";
import { Humanoid2D } from "./humanoid";
            ...
            if (matchData)
            {
                ...
                if (participantsAmount === 2)
                {
                    if (otherParticipant?.Character)
                        this.humanoidController.Rotate(CFrame.lookAt(this.character.GetPivot().Position, otherParticipant.Character.GetPivot().Position).LookVector.mul(new Vector3(1,0,1)));
                ...
                } else print("bad match participant count:", participantsAmount, `match id: ${MatchId} / ${Players.GetPlayers().mapFiltered((n) => n.GetAttribute("MatchId")).join(":: ")}`);
            } else print("no match data");
        } else print("no arena");
    }
